...
- 이런 패턴들을 통해 주문 흐름 자체가 새로운 정보를 제공할 수 있다고 생각해볼 수 있는데, 
- Chapter 3.3을 통해 이런 직관을 공식화 하고자 하는데, 호가 스프레드의 가격 변화를 주문 흐름과 연관시키고자 함
- 호가 스프레드는 유동성 공급자가 더 많은 정보를 가진 투자자와 거래하면서 발생하는 손실을 상쇄하기 위해 요구하는 보상. 따라서 유동성 공급자와 수요자 간의 정보 비대칭성으로 시장의 유동성이 결정됨
- 현실에선 유동성 공급자는 주문을 실행하고, 주문 처리 비용을 충당해야 하는데, 이런 요소로 인해 유동성이 결정되기도 함

- 호가 스프레드가 존재하는 또 다른 이유는 딜러들이 위험 회피적이기 때문인데, 거래일 동안 가격 변동으로 인해 유동성 공급자들은 재고 위험에 노출되는가 때문
- 딜러가 롱 포지션을 잡으면, 가격 하락 위험에 노출되기 때문에 재고 위험에 의존적일 수 밖에 없고 Chatper 3.4에서 이에 대해 분석하고자 함

- Chapter 3.5에선, 재고 위험과 딜러의 위험 회피가 호가에 미치는 영향 분석하고, 세 가지 비용에 대해 설명하고 있음
- 본 장에선 유동성 공급자의 세가지 비용 1) 더 나은 정보를 가진 투자자와의 거래비용(역선택 비용) 2) 주문처리 비용 3) 위험자산 보유 비용(재고 비용)에 대해 설명하고 있음
- 유동성 공급자들은 비용을 보상하기 위해 큰 호가 스프레드가 필요하기 때문에, 시장의 유동성이 증가
- 유동성 공급자가 부담하는 다양한 비용이 시장에 서로 다른 영향을 미치기 때문에 이를 구분하는 것이 중요
- 매수 매도 스프레드가 불리한 선택을 하기 때문에 생기는 보상이라고 하면, 시장 질서가 바뀌면 증권 가치의 추정치를 수정해야 함
- 반면 주문처리 비용이나 재고 위험에 대한 보상이라면, 시장 질서 가격 영향이 일시적이기 때문에 시간이 지남에 따라 소멸해야 함
- 실제 관측에 의하면 모든 요소를 갖고 있다고 생각할 수 있음

# 3.2 Price Dynamics and the Efficient Market Hypothesis

- 일부 축구 클럽은 거래소에 상장되어 있고, 경기 결과에 주가가 민감하게 반응함
- 즉, 새로운 정보는 가격 변화의 원천이라 할 수 있는데, 얼마나 빠르게 가치가 반영되는지가 중요함
- EMH는 가격 조정이 즉각적이어야 한다고 말하는데, 이는 다음과 같은 식으로 나타낼 수 있음

$$
pt=\mu_tμ ≡ E(v|\Omega_t)
$$

- 이 때 mu_t는 t 시점에서 증권가치 V에 대한 시장조성자들의 추정치, omega_t는 그들이 이용할 수 있는 정보, E(v|omega)는 v 정보에 대한 기대값
- EMH에서 가격 변화는 상관관계가 없는 마팅게일을 따름. 새로운 정보가 주어지면 가격에 즉각 반영됨
- 시장이 정보효율적이기 위해선, 시장 참여자들이 갖는 정보의 성격에 대해 구체적으로 정의해야 함
- 1) 딜러가 위험 중립적이고 경쟁적, 2) 투자자가 딜러보다 많은 정보를 가지고 있지 않으며 3) 거래 비용이 없음을 가정해야 함
- 위의 상황에선 호가 스프레드가 0이 되고, 가격 변동이 없음
- 위의 가정들은 하지만, 인트라데이 거래의 중요한 측면을 포착하지 못함
- 1) 주간 가격 변동성이 뉴스만으로 설명되기엔 너무 크다는 선행 연구 2) 양의 스프레드가 표준이라는 단순한 사실을 포착하지 못함 3) 주간 가격 변화가 종종 음의 상관관계
- 나머지 챕터에선 이런 가정들을 완화시키고자 함

# 3.3. Price Dynamics with Informative Order Flow

- EMH에서 증권의 가격은 새로운 정보에 의해 지속적으로 업데이트 된다는 것을 가정
- 새로운 정보는 회사 실적이나 거시 경제 뉴스 등 공개 발표에서 생기지만, 일부 투자자들은 좀 더 빠르게 정보를 얻을 수 있음
- 이로 인해 누군가의 주문이 이미 공개된 정보 이상의 정보를 시장에 전달하는 효과
- 때문에 시장 참가자들은 주문 흐름을 고려해서 증권의 가치 추정치를 수정하고, 호가 스프레드의 존재를 설명할 수 있음
- 딜러가 항상 모든 고객만큼 정보를 알지 못하기 때문에, 거래로 인해 손해를 보는 것을 역선택이라 함
- 시장 조성자는 딜러의 희생으로 이익을 얻기 원하는 고객을 끌어들이고자 하는데, 딜러는 때문에 다른 거래자와의 거래를 통해 이득을 얻어야 함
...
- 끝의 가치 v가 theta의 확률을 갖는 이항분포를 따른다고 가정하고, vH나 vL 중 하나로 향함.
...
- 매수 주문이 두 가지 방식으로 일어나는데, 1-pi의 확률로 유동성 거래자가 오고, 1/2ㅇ,; 확률로 구매??
...
- 딜러들이 경쟁력이 있다면 a1은 기대 이익이 0이 될 것이고, ??
